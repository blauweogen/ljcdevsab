...
class au_shortcode{
	...
	function __construct(){
		...
		add_filter('eventon_event_types_update', array($this, 'eventon_event_types_'),10, 2 );
	...
	// @updated: 2015-4-8
		function eventon_event_types_($event_types, $shortcode_args){
			if(empty($shortcode_args['users'])){
				return $event_types;
			}else{
				$event_types[] = 'event_users';
				return $event_types;
			}			
		}
	...
	// Add user IDs field to shordcode basic cal version
		function add_actionuser_fields_to_eventon_basic_cal($array){			
			$array[] = array(
				...
				'placeholder'=>'eg. 8,19',
				'type'=>'text',
				'guide'=>'Show events for only these user IDs',
				'var'=>'users',
				'default'=>'all'
			);
			$array[] = array(
