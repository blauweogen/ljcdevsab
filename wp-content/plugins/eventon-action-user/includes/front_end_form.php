	...
	<?php 	wp_nonce_field( AJDE_EVCAL_BASENAME, 'evoau_noncename' );	?>
		...
		//access control to form
			if($LOGINCHECK ){
				...
				$log_msg = $__001. (sprintf(__(' <br/><a class="evcal_btn" title="%1$s" href="%2$s">%1$s</a>','eventon'), __('Login','eventon'), wp_login_url(get_permalink()) ) );			

				// register new user
					if (get_option('users_can_register')){
						$log_msg.= (sprintf(__(' <a class="evcal_btn" title="%1$s" href="%2$s/wp-login.php?action=register">%1$s</a>','eventon'), __('Register','eventon'), get_bloginfo('wpurl') ) );
					}
				...
				foreach($EACH_FIELD  as $__index=>$ff):
					...
					if(in_array($INDEX, $SELECTED_FIELDS) || in_array($INDEX, $this->au_form_fields('defaults_ar')) ){
						...
						switch($__field_type){
							...
							case 'allday':
								echo "<div class='row'>
									<p class='label'><input id='evoAU_all_day' name='event_all_day' type='checkbox' value='yes' ".( ($EPMV && !empty($EPMV['evcal_allday']) && $EPMV['evcal_allday'][0]=='yes')? 'checked="checked"':'')."/> <label>".eventon_get_custom_language($opt_2, 'evoAUL_001', 'All Day Event', $lang)."</label></p>
							...
							case 'textarea':
								...
								if($field[1]== 'event_description'){
									...
									echo "<div>".wp_editor($default_val, (!empty($field[4])? $field[4]:''), $editor_args)."</div>";
								}else{
								...
								if(count($terms)>0){
									echo "<div class='row'>
										...
										*/
										...
										echo "<span class='evoau_cat_select_field'>";
										foreach($terms as $term){
											echo "<span><input type='checkbox' name='".$__field_id."[]' value='".$term->term_id."' ".( (count($slectedterms) && in_array($term->term_id, $slectedterms))? 'checked="checked"':null )."/> ".$term->name."</span>";
										}
										echo "</span>";
							...
							case 'uiselect':								
								...
								echo "<div class='row evoau_ui'>
									...
									<div class='evoau_exter' style='display:none'>
										<p class='label'><label for='evoau_ui'>Type the External Url</label></p>
										<p><input name='evcal_exlink' class='fullwidth' type='text'/><br/>
										<i><input name='_evcal_exlink_target' value='yes' type='checkbox'/> Open in a new browser tab</i></p>
									</div></div>";
							break;
							case 'captcha':
