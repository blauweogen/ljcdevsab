...
function awpcp_extra_fields_listings_finder() {
    return new AWPCP_ExtraFieldsListingsFinder( $GLOBALS['wpdb'] );
}
...
class AWPCP_ExtraFieldsListingsFinder {
    ...
    public function filter_keyword_conditions( $conditions, $query ) {
        ...
        foreach ( $fields as $field ) {
            if ( strlen( $query['keyword'] ) <= 3 ) {
                ...
                $conditions[] = $this->db->prepare( " `{$field->field_name}` LIKE '%%%s%%' ", $query['keyword'] );
            }
        }
    ...
    private function get_separated_fields( $query ) {
        static $separated_fields = null;

        if ( is_null( $separated_fields ) ) {
            ...
            foreach ( $this->get_fields() as $field ) {
                if ( isset( $query[ $field->field_name ] ) && ! empty( $query[ $field->field_name ] ) ) {
                    $separated_fields['fields-in-query'][] = $field;
                } else {
                    $separated_fields['fields-not-in-query'][] = $field;
                }
            }
        }
    ...
    public function filter_conditions( $conditions, $query ) {
        foreach ( $this->get_fields_in_query( $query ) as $field ) {
            $value = awpcp_array_data( $field->field_name, '', $query );
