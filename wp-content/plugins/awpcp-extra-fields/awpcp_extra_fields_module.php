...
define( 'AWPCP_EXTRA_FIELDS_MODULE_DB_VERSION', '3.5.3' );
define( 'AWPCP_EXTRA_FIELDS_MODULE_REQUIRED_AWPCP_VERSION', '3.5' );
...
require_once( AWPCP_EXTRA_FIELDS_MODULE_DIR . '/install.php' );


class AWPCP_Extra_Fields_Module {
	private static $instance = null;

	private function __construct() {
		$this->installer = new AWPCP_Extra_Fields_Installer();

		$file = AWPCP_EXTRA_FIELDS_MODULE_DIR . '/' . basename(__FILE__);
	...
	public static function instance() {
		if (is_null(AWPCP_Extra_Fields_Module::$instance)) {
			AWPCP_Extra_Fields_Module::$instance = new AWPCP_Extra_Fields_Module();
		}
		return AWPCP_Extra_Fields_Module::$instance;
	}

	public function activate() {
		if ( $this->is_awpcp_installed_and_updated() && $this->is_comptatible_with_installed_awpcp_version() ) {
	...
	private function load_text_domain() {
		awpcp_load_plugin_textdomain( __FILE__, 'awpcp-extra-fields' );
	}

	/**
	 * @since 3.2.7
	 */
	private function is_awpcp_installed_and_updated() {
		global $awpcp, $awpcp_db_version;

		if ( ! $awpcp ) {
			return false;
		} else if ( ! method_exists( $awpcp, 'is_up_to_date' ) || ! $awpcp->is_up_to_date() ) {
			return false;
		} else if ( version_compare( $awpcp_db_version, AWPCP_EXTRA_FIELDS_MODULE_REQUIRED_AWPCP_VERSION ) < 0 ) {
			return false;
		}
	 ...
	 * @since 3.2.7
	 */
	private function is_comptatible_with_installed_awpcp_version() {
		global $awpcp;
		return $awpcp->is_compatible_with( 'extra-fields', AWPCP_EXTRA_FIELDS_MODULE_DB_VERSION );
	}

	public function setup() {
		if ( ! $this->is_awpcp_installed_and_updated() ) {
			add_action( 'admin_notices', array( $this, 'notices' ) );
			return;
		} else if ( ! $this->is_comptatible_with_installed_awpcp_version() ) {
			add_action( 'admin_notices', array( $this, 'not_compatible_notice' ) );
			return;
		}
	...
	public function updated() {
		...
		// not sure what to do. Downgrade is currently not supported.
		$installed = get_option('awpcp-extra-fields-db-version');
		$plugin = AWPCP_EXTRA_FIELDS_MODULE_DB_VERSION;

		return version_compare($installed, $plugin) === 0;
	}

	public function notices() {
        $message = __( 'The AWPCP Extra Fields module requires AWPCP version %s or newer!', 'awpcp-extra-fields' );
        $message = sprintf( $message, AWPCP_EXTRA_FIELDS_MODULE_REQUIRED_AWPCP_VERSION );
     ...
     * @since 3.0.9
     */
    public function not_compatible_notice() {
        if ( function_exists( 'awpcp_module_not_compatible_notice' ) ) {
            echo awpcp_module_not_compatible_notice( 'extra-fields', AWPCP_EXTRA_FIELDS_MODULE_DB_VERSION );
        }
    }

    protected function load_dependencies() {
		require_once( AWPCP_EXTRA_FIELDS_MODULE_DIR . '/includes/class-extra-form-field.php' );
    }
	...
	public function init() {
		...
		// The default "where clause" logic builder for searching custom fields:
		add_filter('extra_fields_search_filter', 'extra_fields_search_where', 1, 1);
	...
	public function register_settings($settings) {
		...
		$settings->add_setting($key, 'display-extra-fields-in-columns', __('Number of columns of Extra Fields to show.', 'awpcp-extra-fields' ), 'select', 1, __('Extra Fields will be shown in the selected number of columns.', 'awpcp-extra-fields' ), array('options' => $options));
...
?>
        ...
        <div class="postbox">
            <h3 class="hndle"><span><?php _e('Extra Field Settings', 'awpcp-extra-fields' ) ?></span></h3>
            <div class="inside">
...
function awpcp_extra_fields_field_value($field, $value) {
	$type = strtolower($field->field_input_type);
	...
	switch ($type) {
		...
		case 'select multiple':
			...
			$value = stripslashes_deep($value);
			if (!is_serialized($value) && !empty($value)) {
				$value = is_array($value) ? $value : explode(',', $value);
		...
		case 'datepicker':
		default:
			$value = stripslashes_deep( awpcp_extra_fields()->parse_field_posted_data( $field, $value ) );
			break;
	}
...
// Added for search capability
function build_extra_fields_search_where() {
	$conditions = awpcp_get_extra_fields_conditions( array(
		'hide_private' => true,
		'context' => 'search'
	) );
	...
	// $fields is an array of custom field names. The names correspond directly to table fields.
	return apply_filters('extra_fields_search_filter', $fields);
}

/**
 * TODO: Perhaps the search should be improved to consider the type of
 * the field being searched for.
 */
function extra_fields_search_where($fields=array()) {
	global $wpdb;

	$data = awpcp_extra_fields()->get_posted_data( array(), 'search' );

	if ( ! is_array( $fields ) && ! empty( $fields ) ) {
		$fields = array( $fields );
	}

	$keywordphrase = awpcp_array_data( 'keywordphrase', '', $data );

	if ( empty( $fields ) && empty( $keywordphrase ) ) {
		return '';
	}

	$conditions_for_fields_with_empty_values = array();

	if ( ! empty( $keywordphrase ) ) {
		$sql = "( ad_title LIKE '%%%s%%' OR ad_details LIKE '%%%s%%' )";
		$conditions_for_fields_with_empty_values[] = $wpdb->prepare( $sql, $keywordphrase, $keywordphrase );
	}
	...
	foreach ($fields as $field) {
		...
		if ( is_array( $value ) && ( isset( $value['min'] ) || isset( $value['max'] ) ) ) {
			...
			foreach ($value as $key => $val) {
				$sets[] = $wpdb->prepare( " $field_name LIKE '%%%s%%' ", trim( $val ) );
			}
			...
			if ( strlen($value) <=4 ) {
				...
				// $sql = ' AND MATCH (' . $field_name . ') AGAINST (\'"%s"\' IN BOOLEAN MODE) ';
			...
			$where .= $wpdb->prepare( $sql, $value );
		} else if ('' == $value && '' != $keywordphrase) {
			if ( strlen($keywordphrase) <= 3 ) {
				$conditions_for_fields_with_empty_values[] = $wpdb->prepare( " $field_name = '%s' ", $keywordphrase );
			} else {
				$conditions_for_fields_with_empty_values[] = $wpdb->prepare( " $field_name LIKE '%%%s%%' ", $keywordphrase );
			}
		}
	}

	if ( count( $conditions_for_fields_with_empty_values ) > 0 && $where != '' ) {
		$where = sprintf(" AND ( ( %s ) %s )", implode(' OR ', $conditions_for_fields_with_empty_values), $where);
	} elseif ( count( $conditions_for_fields_with_empty_values ) > 0 && $where == '' ) {
		$where = sprintf(" AND ( %s )", implode(' OR ', $conditions_for_fields_with_empty_values));
	}
