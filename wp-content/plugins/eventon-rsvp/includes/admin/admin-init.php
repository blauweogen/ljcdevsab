...
class evorsvp_admin{
	...
	// other hooks
		...
		function eventtop_option($array){
			$array['rsvp_options'] = __('RSVP Count','eventon');
			return $array;
		}
	...
	// appearance
		function evoRS_appearance_settings($array){
			...
			$new[] = array('id'=>'evors','type'=>'hiddensection_open','name'=>'RSVP Styles');
	...
	// language settings additinos
		function evoRS_language_additions($_existen){
			$new_ar = array(
				...
				array('label'=>'Whos coming title','name'=>'evoRSL_002a','placeholder'=>'Who is coming to the event'),
				...
				array('label'=>'EventTop','type'=>'subheader'),
					...
					array('label'=>'One guest is going','var'=>'1'),
					array('label'=>'guests are going','var'=>'1'),
				array('type'=>'togend'),
				...
				<?php
					...
					$cutomization_pg_array = array(
						array(
							...
							'tab_name'=>'General',
							'fields'=>array(
								array('id'=>'evors_onlylogu','type'=>'yesno','name'=>'Allow only logged-in in users to submit RSVP',),							
								array('id'=>'evors_onlylog_chg','type'=>'yesno','name'=>'Allow only logged in users to change RSVP',),
								...
								array('id'=>'evors_eventop_rsvp','type'=>'yesno','name'=>'Allow RSVPing with one-click from eventTop to logged-in users','legend'=>'This will show the normal RSVP option buttons for a logged-in user to RSVP to the event straight from the eventtop. This method will only capture user name, email and rsvp status only'),
							...
							'fields'=>array(
								...
								array('id'=>'evcal_fcx','type'=>'subheader','name'=>'HTML Template'),
								array('id'=>'evcal_fcx','type'=>'note','name'=>'To override and edit the email template copy "eventon-rsvp/templates/confirmation_email.php" to  "yourtheme/eventon/templates/email/confirmation_email.php.'),
	...
	// trash rsvp
		public function trash_rsvp($post_id){
			if ( 'evo-rsvp' != get_post_type( $post_id ))
	       	...
	       	// sync count
	       	if(!empty($PMV['e_id']))
	       		$eventon_rs->frontend->functions->sync_rsvp_count($PMV['e_id'][0]);
		}
