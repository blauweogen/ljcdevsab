	...
	function evoRS_metabox_rsvp(){
		...
		<div style='background-color:#ECECEC; padding:15px;'>
			...
			<table id='evors_rsvp_tb' width='100%' class='evo_metatable'>				
				...
				<tr><td><?php _e('Checkin Status','eventon');?>: </td><td><span class='rsvp_ch_st <?php echo $_checkinST;?>' data-status='<?php echo $_checkinST;?>' data-rsvpid='<?php echo $post->ID;?>'><?php echo $checkin_status;?></span></td></tr>
			...
			<table width='100%' class='eventon_settings_table'>
				<tr><td colspan='2'>
					<p class='yesno_leg_line '>
						...
						<label for='evors_show_rsvp'><?php _e('Show current RSVP count for event')?></label>
					</p>
				</td></tr>
				<tr><td colspan='2'>
					<p class='yesno_leg_line '>
				...
				// if notifications enabled show additional emails field
					if(!empty($optRS['evors_notif']) && $optRS['evors_notif']=='yes' ):?>
						...
						<p style='padding:5px 0'>
							<label for='evors_add_emails'><i><?php _e('Additional email addresses to receive email notifications','eventon')?><?php echo $eventon->throw_guide("Set additional email addresses seperated by commas to receive email notifications upon new RSVP reciept", '',false)?></i></label>
				...
				<tr>
					<td colspan='2'><?php _e('Additional Information only visible to loggedin RSVPed guests & in Confirmation Email','eventon');?><?php echo $eventon->throw_guide('Information entered in here will only be visible on front-end once user has RSVPed to the event.');?>
...
/** Save the menu data meta box. **/
	function evoRS_save_meta_data($arr, $post_id){
		$fields = array(
			...
			'_manage_repeat_cap_rs','evors_additional_data',
			'evors_max_active','evors_max_count'
		);
