...
function loadJIGprettyPhoto($) {
	...
	$.fn.prettyPhoto = function(pp_settings) {
		...
		pp_settings = jQuery.extend({
			...
			jig_twitter: '<div class="jig_pp_social_btn jig_pp_twitter" data-url="{unencoded_location_href}" data-text="{twitter_text}"><a class="twitter-share-button" data-count="none" data-url="{unencoded_location_href}" data-text="{twitter_text}">Tweet</a><script>jigPrettyPhotoHandleTwitter();</script></div>',
		...
		*/
		$.prettyPhoto.open = function(event) {
			...
			if(settings.social_tools){
				...
				new_social=new_social.replace(/{unencoded_location_href}/g,location.href);
				...
				new_social=new_social.replace("{f_location_href}",encodeURIComponent(URI.replace('#!', queryStringChar+'_escaped_fragment_=')));
				...
				if(typeof pp_titles[set_position] !== 'undefined'){
					...
					new_social=new_social.replace(/{twitter_text}/g,pp_titles[set_position]);
				}else{
					new_social=new_social.replace(/{twitter_text}/g,document.title);
		...
		*/
		function _getDimensions(imageWidth,imageHeight){
			...
			if(settings.title_position == 'inside'){
				$('.pp_details .pp_bottom_right').removeAttr('style');
				...
				if(	$pp_description.length !== 0 && $pp_description.position().left === 0){
				...
				}else if($pp_details.find('.pp_bottom_right').position().top !== 0){
					...
					if($pp_description.length === 0 || $pp_details.find('.pp_social').height() > 30){
						// If social is in 2 lines OR there is no description but
						$('.pp_details.pp_includes_title').addClass('pp_alt_x_position');
					}else{
					...
					$('.pp_details.pp_includes_title').removeClass('pp_alt_x_position');
					$('.pp_details .pp_nav, .pp_details .pp_description').removeAttr('style');
				}
			}
		...
		function _build_overlay(caller){
			...
			if(isSet && settings.overlay_gallery) {
				...
				for (var i=0; i < pp_images.length; i++) {
					...
					toInject += '<li'+classname+'><a href="#"><img src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///yH5BAEAAAAALAAAAAABAAEAAAIBRAA7" style="background-image: url(' + img_src + ');" alt="' + pp_titles[i] + '" /></a></li>';
				};
	...
	window['jigPrettyPhotoHandleTwitter'] = function(){
		var $ = jQuery;
		if(typeof twttr == 'undefined' || typeof twttr.widgets == 'undefined' ){
			if($('a[src*="platform.twitter.com/widgets.js"]').length == 0){
				$("body").append('<script id="twitter-wjs" type="text/javascript" src="https://platform.twitter.com/widgets.js"></script>');
			}
		}else{
			//twttr.widgets.load([$('.jig_pp_twitter .twitter-share-button')[0]]);
			if(typeof __twttr.widgets.loaded !== 'undefined'){
				__twttr.widgets.loaded = false;
			}
			twttr.widgets.load();
		}

		/*
			sooner or later __twttr.widgets.loaded will be true
			setinterval check
			once it's true..
		*/
		var checkForTwitterWidgetLoadedInterval = window.setInterval(function(){
			if(typeof __twttr !== 'undefined' && typeof __twttr.widgets.loaded !== 'undefined'){
				window.clearInterval(checkForTwitterWidgetLoadedInterval);
				var twitterButton = $('.jig_pp_twitter iframe.twitter-share-button');
				if(twitterButton.length !== 0){
					twitterButton.on('load',function(){

						// if render has failed
						if(twitterButton.length !== 0 && (
								(twitterButton.width() == 300 && twitterButton.height() == 150) ||
								(twitterButton.width() == 0 && twitterButton.height() == 0)
							)){
							//enforceTweetButton
							var srcCache = twitterButton.attr('src');
							twitterButton.off('load').on('load',function(){
								twitterButton.off('load').attr( 'src', srcCache);
								enforceTweetButton();
	...
	var enforceTwitterCount = 0;
	// checks for twitter button, once the prettyPhoto modal is open (called near the callback)
	function enforceTweetButton(){
		var twitterButton = $('.jig_pp_twitter iframe.twitter-share-button'),
			twitterParent = $('.jig_pp_twitter');
		if(enforceTwitterCount < 10 && twitterButton.length !== 0 && (
								(twitterButton.width() == 300 && twitterButton.height() == 150) ||
								(twitterButton.width() == 0 && twitterButton.height() == 0)
			)){
				var dataURL = twitterParent.attr('data-url'),
					dataText = twitterParent.attr('data-text');
			$('.jig_pp_twitter').empty().append('<a class="twitter-share-button" data-count="none" data-url="'+dataURL+'" data-text="'+dataText+'">');
			setTimeout(function() {
				twttr.widgets.load();
				$('.jig_pp_twitter iframe.twitter-share-button').one('load',function(){
					setTimeout(function() {
						enforceTweetButton();
					}, 200);
				});
